...
`}Jt.push(f)}};let ce,lt;function Rn(){ce=new(window.AudioContext||window.webkitAudioContext),window.addEventListener("mousedown",Ut),window.addEventListener("touchstart",Ut),window.addEventListener("keydown",Ut),lt=ce.createGain(),lt.gain.value=.05,lt.connect(ce.destination)}window.playTone=(n,x,R)=>{const P=ce.createOscillator();P.type="square",P.frequency.value=n,P.start(R),P.stop(R+x),P.connect(lt)};window.stopTone=()=>{oscillator.stop()};window.getAudioTime=()=>ce.currentTime;function Ut(){ce.resume()}window.Module={onRuntimeInitialized:Gn};const Bl=document.createElement("script");Bl.setAttribute("src","./wasm/cglp.js");document.head.appendChild(Bl);
